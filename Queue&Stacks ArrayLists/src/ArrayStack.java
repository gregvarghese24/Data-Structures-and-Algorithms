    ...
    public void push(T data) {
        if (data == null) {
            throw new IllegalArgumentException("The data provided"
                    + " is nonexistent or null");
        }

        if (size >= backingArray.length) {
            //increase backing array size
            //create for loop
            T[] tempArray = (T[]) new Object[backingArray.length * 2];
            for (int n = 0; n < backingArray.length; n++) {
                tempArray[n] = backingArray[n];
            }
    ...
    public T pop() {
        if (size == 0) {
            throw new NoSuchElementException("The "
                    + "stack contains nothing in it. "
                    + "Try using a Array with 1 or more content(s)");
        }


        T newData = backingArray[size - 1 ];
